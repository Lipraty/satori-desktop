import type { Context, PackageJson } from 'yakumo'
...
export function pluginImport(manifest: PluginManifest[]) {
  return `// this file is auto generated by yakumo, do not edit it manually.
...
export async function bundlePlugins(ctx: Context) {
  ...
  const loaderPluginsPath = `${ctx.yakumo.cwd}/packages/loader/src/plugins.ts`
  const manifest: PluginManifest[] = []

  for (const path of paths) {
    const root = ctx.yakumo.cwd + path
    const meta = ctx.yakumo.workspaces[path]
    if (meta?.sapp && !meta.sapp.node)
      continue
  ...
  try {
    await access(loaderPackagePath)
    await access(loaderPluginsPath)
  }
  catch {
    ctx.logger('builder').warn('skip loader plugin injection: packages/loader is not present in current workspace.')
  ...
  await writeFile(resolve(loaderPluginsPath), pluginImport(manifest), 'utf-8')
  ctx.logger('builder').info('bundle plugins [%s] success', manifest.map(plugin => plugin.name).join(', '))

import { Context, CommunicationAdapter, CommunicationRequest, CommunicationResponse } from '@satoriapp/webui'

export class WebAdapter extends CommunicationAdapter {
  ...
  private connectWebSocket() {
    ...
    this.ws.onmessage = (event) => {
      const response: CommunicationResponse = JSON.parse(event.data)
  ...
  async action<T = any, R = any>(request: CommunicationRequest<R>): Promise<CommunicationResponse<T>> {

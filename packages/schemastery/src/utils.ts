...
export function getChoices(schema: Schema) {
  const inner: Schema[] = []
  const choices = schema.list.filter((item: Schema) => {
    if (item.meta.hidden)
      return
...
export function explain(schema: Schema, value: any): readonly [string, any[]?] | undefined {
  if (isNullable(value))
    return
  if (schema.type !== 'string')
    return
...
export function useModel<T = any>(options?: ConfigOptions<T>) {
  ...
  const doWatch = () => watch(config, (value: T) => {
    try {
      if (options?.output)
        value = options.output(value)
      const schema = optional(Schema(props.schema))
      if (deepEqual(schema(value), props.schema.meta.default, options?.strict))
        value = null
    }
    catch {
      return
...
export function useEntries() {
  ...
  const isMin = computed(() => entries.value.length <= props.schema.meta.min)
...
function isConstUnion(schema: Schema) {
  return schema.type === 'union' && schema.list.every((item: Schema) => item.type === 'const')
}
  ...
  return {
    ...
    toggle(key: string) {
      if (values.value.includes(key)) {
        values.value = values.value.filter((k: string) => k !== key)
      }
      else {
...
function ensureColumns(entries: [string, Schema][]): [string, Schema][] | undefined {
  entries = entries.filter(([, schema]) => !schema.meta.hidden)
  if (entries.every(([, schema]) => isValidColumn(schema)))
    return entries
}

export function toColumns(schema: Schema): [string | null, Schema][] | undefined {
